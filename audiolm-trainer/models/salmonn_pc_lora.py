...
from .lora import LoRALinear, apply_lora, compute_kl_div_loss, compute_feature_distillation_loss


class SALMONN_PCLORA(nn.Module):
    @classmethod
    ...
    def __init__(
        ...
        self.pc_lora = pc_lora
        self.pc_eval = pc_eval
        self.alpha = 0.3
        ...
        if not only_preprocessor:
            ...
            if self.pc_eval:
                apply_lora(self.llama_model, r=lora_rank, lora_alpha=lora_alpha, lora_dropout=lora_dropout, target_modules=self.target_modules)
                logging.info('PC LoRA Evaluation')
            elif self.lora:
            ...
            # PC-LoRA
            if self.pc_lora and not self.pc_eval:
                # Teacher 모델 고정
                ...
                if self.low_resource:
                        ...
                        # [ ] llama_path모델 구조 다르게 변경할수있게 수정 필요
                        self.student_llama = AutoModelForCausalLM.from_pretrained(
                            llama_path,
                            torch_dtype=torch.float16,
                            token=token,
                            quantization_config=quantization_config,
                ...
                self.student_llama.resize_token_embeddings(len(self.llama_tokenizer))
                for name, param in self.student_llama.named_parameters():
                    param.requires_grad = False
                logging.info('Loading Student LLaMA Done')

                if self.lora:
                    apply_lora(self.student_llama, r=lora_rank, lora_alpha=lora_alpha, lora_dropout=lora_dropout, target_modules=self.target_modules)
                    logging.info('Applied LoRA to student_llama')
    ...
    def prompt_wrap(self, embeds, atts, prompt, multi_prompt=False):
        if prompt:
            if multi_prompt:
                ...
                ### test ###
                #p_before_embeds = self.llama_model.model.embed_tokens(p_before_tokens.input_ids) if not self.lora or self.pc_eval else self.llama_model.model.model.embed_tokens(p_before_tokens.input_ids)
                ...
                ### test ###
                #p_after_embeds = self.llama_model.model.embed_tokens(p_after_tokens.input_ids) if not self.lora or self.pc_eval else self.llama_model.model.model.embed_tokens(p_after_tokens.input_ids)
                ...
                ### test ###
                # p_before_embeds = self.llama_model.model.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1, -1) if not self.lora or self.pc_eval else self.llama_model.model.model.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1, -1)
                # p_after_embeds = self.llama_model.model.embed_tokens(p_after_tokens.input_ids).expand(batch_size, -1, -1) if not self.lora or self.pc_eval else self.llama_model.model.model.embed_tokens(p_after_tokens.input_ids).expand(batch_size, -1, -1)
    ...
    def forward(self, samples, alpha_decay:float, verbose=False):
        ...
        ### test ###
        # to_regress_embeds = self.llama_model.model.embed_tokens(to_regress_tokens.input_ids) if not self.lora or self.pc_eval else self.llama_model.model.model.embed_tokens(to_regress_tokens.input_ids)
        ...
        ### test ###
        # bos_embeds = self.llama_model.model.embed_tokens(bos) if not self.lora or self.pc_eval else self.llama_model.model.model.embed_tokens(bos)
        ...
        with self.maybe_autocast():
            if self.pc_lora and not self.pc_eval:
                student_outputs = self.student_llama(
                ...
                #kl_loss = compute_kl_div_loss(student_outputs.logits, outputs.logits)
                mse_loss = compute_feature_distillation_loss(self.student_llama, self.llama_model, self.target_modules)
                loss = self.alpha * task_loss + (1 - self.alpha) * mse_loss
            
                # Update lambda_factor for LoRALinear layers
                for module in self.student_llama.modules():
                    if isinstance(module, LoRALinear):
                        module.lambda_factor = alpha_decay
                        test_alpha = module.lambda_factor
                ...
                if self.pc_eval:
                    ...
                    for module in self.llama_model.modules():
                        if isinstance(module, LoRALinear):
                            if self.inference:
                                module.lambda_factor = 0.0 # inference mode
                            test_alpha = module.lambda_factor
                ...
                logging.info(f"loss : {loss} | lambda_factor : {test_alpha}")
        ...
        return loss, task_loss, mse_loss
    ...
    def generate(self, samples, generate_cfg, prompts=None):
        ...
        if self.pc_lora and not self.pc_eval:
            outputs = self.student_llama.generate(
    ...
    def from_config(cls, config):
        ...
        pc_lora = config.get("pc_lora", False)
        pc_eval = config.get("pc_eval", False)
